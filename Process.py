...
class Process(VideoCapture):
    ...
    def data_frame(self):
        ...
        self.df = pd.DataFrame(index=range(int(self.frames_count)))
    ...
    def Process(self):
        ...
        ret, frame = self.cap.read ()  # import image
        ratio = 1  # resize ratio
        image = cv2.resize (frame, (0, 0), None, ratio, ratio)  # resize image
        width2, height2, channels = image.shape
        video = cv2.VideoWriter ('traffic_counter.avi', cv2.VideoWriter_fourcc ('M', 'J', 'P', 'G'), self.fps,
                                 (height2, width2),
                                 1)
        ...
        while True:
            ...
            if ret:  # if there is a frame continue with code
                ...
                lineypos = 225
                cv2.line (image, (0, lineypos), (int (self.width), lineypos), (255, 0, 0), 5)
                ...
                if len (cxx):  # if there are centroids in the specified area
                    ...
                    if not carids:  # if carids is empty
                        ...
                        for i in range (len (cxx)):  # loops through all centroids
                            ...
                            self.df[str (carids[i])] = ""  # adds a column to the dataframe corresponding to a carid

                            # assigns the centroid values to the current frame (row) and carid (column)
                            self.df.at[int (framenumber), str (carids[i])] = [cxx[i], cyy[i]]
                        ...
                        for i in range (len (cxx)):  # loops through all centroids
                            ...
                            for j in range (len (carids)):  # loops through all recorded car ids
                                ...
                                oldcxcy = self.df.iloc[int (framenumber - 1)][str (carids[j])]
                        ...
                        for j in range (len (carids)):  # loops through all current car ids
                            ...
                            if mindx == 0 and mindy == 0 and np.all (dx[:, j] == 0) and np.all (dy[:, j] == 0):
                                ...
                                if np.abs (mindx) < maxrad and np.abs (mindy) < maxrad:
                                    ...
                                    self.df.at[int (framenumber), str (carids[j])] = [cxx[minx_index], cyy[miny_index]]
                                    minx_index2.append (
                        ...
                        for i in range (len (cxx)):  # loops through all centroids
                            ...
                            if i not in minx_index2 and miny_index2:
                                ...
                                self.df[str (totalcars)] = ""  # create another column with total cars
                                ...
                                self.df.at[int (framenumber), str (t)] = [cxx[i], cyy[i]]  # add centroid to the new car id
                ...
                for i in range (len (carids)):  # loops through all carids
                    ...
                    if self.df.at[int (framenumber), str (carids[i])] != '':
                ...
                for i in range (currentcars):  # loops through all current car ids on screen
                    ...
                    curcent = self.df.iloc[int (framenumber)][str (carids[currentcarsindex[i]])]

                    # grabs centroid of certain carid for previous frame
                    oldcent = self.df.iloc[int (framenumber - 1)][str (carids[currentcarsindex[i]])]
                    ...
                    if curcent:  # if there is a current centroid
                        ...
                        cv2.putText (image, "Centroid" + str (curcent[0]) + "," + str (curcent[1]),
                                     (int (curcent[0]), int (curcent[1])), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 255, 255),
                                     2)
                        ...
                        if oldcent:  # checks if old centroid exists
                            ...
                            cv2.rectangle (image, (int (xstart), int (ystart)), (int (xwidth), int (yheight)),
                                           (0, 125, 0), 1)
                ...
                cv2.putText (image, "Cars in Area: " + str (currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5,
                             (0, 170, 0), 1)
                ...
                cv2.putText (image, "Cars Crossed Down: " + str (carscrosseddown), (0, 45), cv2.FONT_HERSHEY_SIMPLEX,
                             .5,
                             (0, 170, 0), 1)
                ...
                cv2.putText (image, "Frame: " + str (framenumber) + ' of ' + str (self.frames_count), (0, 75),
                             cv2.FONT_HERSHEY_SIMPLEX,
                             .5, (0, 170, 0), 1)
    ...
    def later(self):
        ...
        self.df.to_csv('traffic.csv', sep=',')
